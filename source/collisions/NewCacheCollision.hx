...
class NewCacheCollision implements ICollision {
	...
	public function pixelPerfectCheck(Contact:FlxSprite, Target:FlxSprite, AlphaTolerance:Int = 255, ?Camera:FlxCamera):Bool
	{
		...
		{
		#if flash
			boundsA.setTo(pointA.x, pointA.y, Contact.framePixels.width, Contact.framePixels.height);
			boundsB.setTo(pointB.x, pointB.y, Target.framePixels.width, Target.framePixels.height);
		#else
		...
		boundsA.setTo(-matrixA.tx, -matrixA.ty, overlapWidth, overlapHeight);
		boundsB.setTo(-matrixB.tx, -matrixB.ty, overlapWidth, overlapHeight);
		...
		debug = overlapArea;
		overlapArea.rect.width = intersect.width;
		overlapArea.rect.height = intersect.height;
