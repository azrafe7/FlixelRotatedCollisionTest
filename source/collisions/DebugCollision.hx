...
class DebugCollision implements ICollision {
	...
	//#end
		...
		var overlapArea:BitmapData = new BitmapData(overlapWidth, overlapHeight, true, 0);
		...
		inline function assert(cond:Bool):Void 
		{
			if (!cond) {
				trace( -matrixA.tx, -matrixA.ty, boundsA);
				trace( -matrixB.tx, -matrixB.ty, boundsB);
				trace( pixelsA.length/4, pixelsB.length/4);
				throw "Assert";
			}
		}
		
		if (FlxG.keys.pressed.C) assert(pixelsA.length == pixelsB.length);
		...
		for (y in 0...overlapHeight) 
		{
			for (x in 0...overlapWidth) 
			{
				...
				alphaA = pixelsA[idx];
				alphaB = pixelsB[idx];
				overlapArea.setPixel32(x, y, ((0xFF0000 * (alphaA > 0 ? 1:0)) | alphaA << 24) | ((0x0000FF * (alphaB > 0 ? 1:0)) | alphaB << 24));
